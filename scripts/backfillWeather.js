...
async function getFixturesWithoutWeather(startYear, endYear, limit = 50, offset = 0) {
  ...
  const query = `
    ...
    WHERE starting_at >= $1 AND starting_at < $2
      AND NOT EXISTS (SELECT 1 FROM public.fixture_weather WHERE fixture_id = public.fixtures.id)
...
async function backfillWeather(startYear = 2023, endYear = 2025, batchSize = 50) {
  let offset = 0;
  let totalProcessed = 0;
  let totalErrors = 0;

  while (true) {
    const fixtureIds = await getFixturesWithoutWeather(startYear, endYear, batchSize, offset);
    ...
    offset += batchSize;
    await new Promise(resolve => setTimeout(resolve, 1000)); // 1s delay for rate limit
  }
...
backfillWeather(parseInt(startYear) || 2023, parseInt(endYear) || 2025);
