-- Update view analytics.v_fixtures_upcoming: árbitro pelas últimas 3 temporadas na mesma liga (fallback liga/temporada atual)
...
team_avgs as (
  select h.team_id, h.season_id, h.league_id,
         ...
         sum(coalesce(ptf.pts_total,0)) as it1_sum,
         sum(coalesce(pft.pts_total,0) - coalesce(ptf.pts_total,0)) as it2_sum,
         sum(coalesce(ptf.pts_1h,0)) as it1_1h_sum,
         sum(coalesce(pft.pts_1h,0) - coalesce(ptf.pts_1h,0)) as it2_1h_sum,
         count(*) as games
...
-- últimas 3 temporadas por liga (com base no último jogo registrado)
...
-- fallback: média do árbitro na liga/temporada do jogo
...
select
  ...
  (to_char(coalesce(ta_h.it1_sum,0)::numeric(10,2), 'FM999990.00') || ' - ' || to_char(coalesce(ta_h.it2_sum,0)::numeric(10,2), 'FM999990.00')) as home_diff,
  (to_char(coalesce(ta_h.it1_1h_sum,0)::numeric(10,2), 'FM999990.00') || ' - ' || to_char(coalesce(ta_h.it2_1h_sum,0)::numeric(10,2), 'FM999990.00')) as home_diff_1h,
  ...
  (to_char(coalesce(ta_a.it1_sum,0)::numeric(10,2), 'FM999990.00') || ' - ' || to_char(coalesce(ta_a.it2_sum,0)::numeric(10,2), 'FM999990.00')) as away_diff,
  (to_char(coalesce(ta_a.it1_1h_sum,0)::numeric(10,2), 'FM999990.00') || ' - ' || to_char(coalesce(ta_a.it2_1h_sum,0)::numeric(10,2), 'FM999990.00')) as away_diff_1h,
  coalesce(ta_a.games,0) as away_j,
  -- Árbitro (últimas 3 temporadas por liga; fallback liga/temporada do jogo)
  mr.referee_id,
  r.display_name as referee_name,
  coalesce(case when coalesce(ra3.games,0) >= 10 then ra3.ref_t else rls.ref_t end,0)::numeric(10,2) as referee_t,
  coalesce(case when coalesce(ra3.games,0) >= 10 then ra3.ref_t_1h else rls.ref_t_1h end,0)::numeric(10,2) as referee_t_1h,
  coalesce(case when coalesce(ra3.games,0) >= 10 then ra3.games else rls.games end, 0) as referee_j
from targets tgt
...
comment on view analytics.v_fixtures_upcoming is 'Jogos de hoje (BRT) em ligas selecionadas; times: médias por liga/temporada atual; árbitro: últimas 3 temporadas da liga (fallback temporada atual).';
