...
-- Ajustes: nomes dos times (home_team_name/away_team_name) e médias de árbitro pelos últimos 50 jogos (fallback para média por liga/temporada)
...
-- Histórico de partidas por time na mesma liga/temporada dos alvos (apenas partidas passadas)
...
team_avgs as (
  -- Média por time dentro da (liga, temporada)
...
main_referee as (
  -- Escolhe um árbitro principal por fixture (primeiro da lista)
...
ref_recent as (
  -- Últimos 50 jogos de cada árbitro (qualquer liga/temporada)
...
ref_league_season as (
  -- fallback: média dentro da mesma (liga, temporada)
...
select
  ...
  -- Home DIFFs (totais de pontos na métrica de cartões)
  (coalesce(ta_h.it1_all,0) - coalesce(ta_h.it2_all,0))::numeric(10,2) as home_diff,
  (coalesce(ta_h.it1_1h,0) - coalesce(ta_h.it2_1h,0))::numeric(10,2) as home_diff_1h,
  -- Home J (número de jogos usados nas médias)
  coalesce(ta_h.games,0) as home_j,
  -- Away team metrics (por nome)
  ...
  -- Away DIFFs
  (coalesce(ta_a.it1_all,0) - coalesce(ta_a.it2_all,0))::numeric(10,2) as away_diff,
  (coalesce(ta_a.it1_1h,0) - coalesce(ta_a.it2_1h,0))::numeric(10,2) as away_diff_1h,
  -- Away J
  ...
  -- Árbitro J (número de jogos considerados)
...
comment on view analytics.v_fixtures_upcoming is 'Jogos restantes da semana (UTC) com métricas de cartões por time (tomados/provocados) e árbitro; regras: amarelo=1, vermelho direto=2, segundo amarelo=2; 1h = minutos <=45. Árbitro usa média dos últimos 50 (>=10) com fallback para liga/temporada.';
